...
AddonTitle=main.AddonTitle; wizardUrl='http://tribeca.tvaddons.ag/tools/tvmc/'; 
SiteDomain='TVADDONS.AG'; TeamName='TVMC'; 
...
def HELPCATEGORIES():
        ...
        if '://' in link: 
        	...
        	main.addHELPDir2('Click Here to Finalize TVMC Installation','TVMC.Android.Package',link,'helpwizard','','',"Wizard Package for Android","main")
        main.doSetView('50')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
        ## ### ## 
...
def HELPWIZARD(name,url,description,filetype):
    ...
    if confirm.yesno(TeamName,"You're almost done!","Just click Continue to finalize ","the installation of TVMC.","Exit","Continue"):
        try:
            ...
            if mode=="service": dialog.ok(TeamName,"TVMC has now been successfully installed."," ","Please visit www.tvaddons.ag for future support.")
            else: dialog.ok(TeamName,"TVMC has now been successfully installed."," ","Please visit www.tvaddons.ag for future support.")
            try:
            ...
            ##
        except: main.setts('status','failed'); dialog=xbmcgui.Dialog(); dialog.ok(TeamName,"Problem!  Installation has failed,","please Restart TVMC to try again.","")
#==========
